...
- 응답 필드 설명:
  ...
  - `poster_url`: 포스터 이미지 절대 경로. 공유 카드의 배경으로 사용.
  - `director`, `cast`, `genre`, `distributor`: 프론트 카드/상세 응답에 바로 바인딩하므로 배열 또는 콤마 문자열 형태로 반환.
...
- `projects` 테이블 예시:
  ...
  - `poster_url`: TMDb 포스터 이미지 경로(필요 시 CDN URL) 저장.
...
- 커뮤니티 리액션(같이 기다려요) 저장은 별도 `reactions` 또는 `project_reactions` 테이블을 둔다. 스키마 예시: `project_id`, `user_id`(선택), `reaction_type`, `count`, `updated_at`. 익명 합산만 필요하면 `project_id` 단일 행으로 관리.
- `/dday` 조회 응답에는 `reaction_count` 필드를 포함해 프론트 배지가 서버 수치를 반영하도록 한다.
...
- 모든 성공 응답은 매번 D-Day 문자열을 재계산해 전달한다(오늘 날짜 기준). 이미 저장된 레코드를 반환하더라도 UI 배지는 최신 남은 일수를 사용해야 한다.
...
## 조회 API와 Longest D-Day
- `GET /dday`: 저장된 모든 프로젝트를 최신 D-Day, 메타데이터, `reaction_count`와 함께 반환. 정렬은 최근 갱신 순.
- `GET /dday/longest`: 가장 멀리 있는 미래 개봉일을 계산해 반환. 모두 과거라면 `detail`에 "재개봉 일정 없음" 메시지를 포함하고 404 처리 또는 빈 응답을 정의.
- 두 엔드포인트 모두 SQLite → PostgreSQL 전환 시 대량 정렬/집계가 가능하도록 SQL 단에서 `ORDER BY release_date`를 수행한다.
