...
- OpenAI 키가 없을 땐 TMDb 직접 조회로 대체하며, 그래도 동일하게 `tool_started`/`movie`/`token`/`final` 이벤트를 만들어 프론트의 단계 표시 로직이 깨지지 않도록 한다. 드라마/시리즈 요청 시에는 `tv_search` 툴을 사용해 동일한 이벤트 구조로 응답하며, `tool_result`/`dday` 이벤트에는 `content_type` 정보를 포함해 후속 처리가 가능하다.
